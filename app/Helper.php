...
class Helper {
	...
	public static function get_posts( $args = [], $show_heading = false, $show_cached = false ) {
		...
		// use cache
		if( true === $show_cached && ( $cached_posts = wp_cache_get( "universal-blocks_{$_args['post_type']}", 'universal-blocks' ) ) ) {
			$posts = $cached_posts;
		}
		...
		// don't use cache
			...
			wp_cache_add( "universal-blocks_{$_args['post_type']}", $posts, 'universal-blocks', 3600 );
		}

		$posts = $show_heading ? [ '' => sprintf( __( '- Choose a %s -', 'universal-blocks' ), $_args['post_type'] ) ] + $posts : $posts;

		return apply_filters( 'wppub_get_posts', $posts, $_args );
	}
	 ...
	 *
	 * It'll look into /universal-blocks directory of your active theme
	 * first. if not found, default template will be used.
	 * can be overwriten with universal-blocks_template_overwrite_dir hook
	 *
	...
	public static function get_template( $slug, $base = 'views', $args = null ) {
		...
		$overwrite_template_dir = apply_filters( 'wppub_template_overwrite_dir', get_stylesheet_directory() . '/universal-blocks/', $slug, $base, $args );
		
		// default template directory
		$plugin_template_dir = dirname( UNIVERSAL_BLOCKS_FILE ) . "/{$base}/";
		...
		elseif ( file_exists( $plugin_template_path ) ) {
			...
			return __( 'Template not found!', 'universal-blocks' );
		}
	}
}